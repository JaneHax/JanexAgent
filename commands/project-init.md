---
description: Detect a project's stack and produce a dry-run Multiversal onboarding plan using the repository's install manifests and stack mappings.
---

# /project-init

Create a safe, reviewable Multiversal onboarding plan for the current project. This command should start in dry-run mode and only write files after explicit user approval.
...
3. Use Multiversal's installer and manifest tooling. Do not hand-copy files or clone arbitrary remotes as an install shortcut.
...
- Multiversal config: `ecc-install.json`
- optional stack map: `config/project-stack-mappings.json` in the Multiversal repo

When the Multiversal checkout is available, use `config/project-stack-mappings.json` as the stack-to-rules/skills reference. If the file is unavailable, fall back to the installed Multiversal manifests and explicit user choices.
...
3. Resolve the smallest useful Multiversal plan:
...
- `/multiversal-guide` for interactive feature discovery before installing
