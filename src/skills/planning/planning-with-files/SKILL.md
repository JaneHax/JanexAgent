...
|---|---|---|
| `/plugin marketplace add planning-with-files` then `/plugin install` | SKILL.md, scripts, templates, **plus `commands/` folder** | Yes, as `/plan-goal` and `/plan-loop` |
| `npx skills add planning-with-files` (or ClawHub) | SKILL.md, scripts, templates only | No, follow the manual fallback below |
